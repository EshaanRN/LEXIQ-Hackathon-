...
function NavLink({ to, icon, label, disabled }: { to: string; icon: React.ReactNode; label: string; disabled: boolean }) {
  return (
    <Link
      ...
      viewTransition
      disabled={disabled}
      className="group relative flex min-w-14 flex-col items-center gap-1 rounded-xl px-3 py-1.5 text-muted-foreground transition-all duration-200 ease-out hover:text-foreground hover:-translate-y-0.5 active:scale-95 disabled:pointer-events-none disabled:opacity-70"
      activeProps={{
        className:
